...
Game::Game()
	...
	, mpOrbitActor(nullptr)
	, mpSplineActor(nullptr)
	, mpStartSphere(nullptr)
	, mpEndSphere(nullptr)
	, mpCrosshair(nullptr)
{
}
...
bool Game::Initialize()
{
	...
	// LoadData
	{
		...
		Actor* const pCrosshair = new Actor(this);
		pCrosshair->SetScale(2.f);

		mpCrosshair = new SpriteComponent(pCrosshair);
		...
		mpFollowActor = new FollowActor(this);
		mpOrbitActor = new OrbitActor(this);
		mpSplineActor = new SplineActor(this);

		changeCamera('1');

		mpStartSphere = new Actor(this);
		mpStartSphere->SetPosition(Vector3(100000.f, 0.f, 0.f));
		mpStartSphere->SetScale(0.25f);

		MeshComponent* const pStartSphere = new MeshComponent(mpStartSphere);
		pStartSphere->SetMesh(mpRenderer->GetMesh("Assets/Sphere.gpmesh"));

		mpEndSphere = new Actor(this);
		mpEndSphere->SetPosition(Vector3(10000.f, 0.f, 0.f));
		mpEndSphere->SetScale(0.25f);

		MeshComponent* const pEndSphere = new MeshComponent(mpEndSphere);
		pEndSphere->SetMesh(mpRenderer->GetMesh("Assets/Sphere.gpmesh"));
		pEndSphere->SetTextureIndex(1);
	}
	...
	case SDL_BUTTON_LEFT:
		{
			...
			const Vector3 start = mpRenderer->Unproject(screenPoint);
			screenPoint.Z = 0.9f;

			const Vector3 end = mpRenderer->Unproject(screenPoint);

			mpStartSphere->SetPosition(start);
			mpEndSphere->SetPosition(end);
		}
		break;
...
{
	mpFPSActor->SetState(Actor::EState::PAUSED);
	mpFPSActor->SetVisible(false);
	mpCrosshair->SetVisible(false);

	mpFollowActor->SetState(Actor::EState::PAUSED);
	mpFollowActor->SetVisible(false);

	mpOrbitActor->SetState(Actor::EState::PAUSED);
	mpOrbitActor->SetVisible(false);

	mpSplineActor->SetState(Actor::EState::PAUSED);

	switch (mode)
	{
	case '1':
		mpFPSActor->SetState(Actor::EState::ACTIVE);
		mpFPSActor->SetVisible(true);
		mpCrosshair->SetVisible(true);
		break;

	case '2':
		mpFollowActor->SetState(Actor::EState::ACTIVE);
		mpFollowActor->SetVisible(true);
		break;

	case '3':
		mpOrbitActor->SetState(Actor::EState::ACTIVE);
		mpOrbitActor->SetVisible(true);
		break;

	case '4':
		mpSplineActor->SetState(Actor::EState::ACTIVE);
		mpSplineActor->RestartSpline();
		break;
